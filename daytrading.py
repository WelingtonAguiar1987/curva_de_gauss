...
st.markdown("<h1 style='text-align: center; color: grey;'>Variações</h1>", unsafe_allow_html=True)
...
with col_2:
    ...
    ultimo_preco_percentual = ((ultimo_preco - preco_ultimo_fechamento) / preco_ultimo_fechamento) * 100
...
for index, row in df_dp.iterrows():
    color = 'red' if row['VALOR POR DP'] > 0 else 'green'  # Cor baseada no valor do desvio padrão
    preco_dp = preco_ultimo_fechamento + row['VALOR POR DP']
    percentual_variacao = ((preco_dp - preco_ultimo_fechamento) / preco_ultimo_fechamento) * 100
    descricao_dp = (f'DP {row["DESVIOS PADRÕES"]}: {preco_dp:.2f} ({percentual_variacao:.2f}%)')  # Adicionando o percentual de variação
...
descricao_ultima_abertura = (f'Última Abertura: {ultima_abertura:.2f} ({variacao_gap_percentual:.2f}%)')
...
descricao_ultimo_preco = (f'Último Preço: {ultimo_preco:.2f} ({ultimo_preco_percentual:.2f}%)')
...
# BOTÕES DE LINKS A SITES:
st.sidebar.subheader('Sites para Consultar Informações:')
botao_tradingeconomics = st.sidebar.button('Acessar o Trading Economics') 
botao_tradingeconomics_nasdaq = st.sidebar.button('Acessar o Nasdaq Trading Economics') 
botao_tradingeconomics_vix = st.sidebar.button('Acessar o VIX Trading Economics')  
botao_previsoes_tradingeconomics = st.sidebar.button('Acessar Previsões T.Economics') 
botao_financial_juice = st.sidebar.button('Acessar o Financial Juice')
botao_cnbc_bonds = st.sidebar.button('Acessar o CNBC Bonds e Rates')   
botao_finviz = st.sidebar.button('Acessar o Finviz Cesta Tecnologia')     
botao_moscca = st.sidebar.button('Acessar o Moscca Panorama Global')   
botao_cme_nasdaq = st.sidebar.button('Acessar Dados de Vencimentos Futuros de Nasdaq - CME')   
 

if botao_tradingeconomics:
    webbrowser.open_new_tab('https://tradingeconomics.com/')
        
if botao_tradingeconomics_nasdaq:
    webbrowser.open_new_tab('https://tradingeconomics.com/ndx:ind')

if botao_tradingeconomics_vix:
    webbrowser.open_new_tab('https://tradingeconomics.com/vix:ind')    

if botao_previsoes_tradingeconomics:
    webbrowser.open_new_tab('https://tradingeconomics.com/united-states/forecast')
 
if botao_financial_juice:
    webbrowser.open_new_tab('https://www.financialjuice.com/home')   
        
if botao_cnbc_bonds:
    webbrowser.open_new_tab('https://www.cnbc.com/bonds/')     
        
if botao_finviz:
    webbrowser.open_new_tab('https://finviz.com/map.ashx?t=sec#google_vignette')   
 
if botao_moscca:
    webbrowser.open_new_tab('https://www.moscca.com.br/panorama/layout1.php')   

if botao_cme_nasdaq:
    webbrowser.open_new_tab('https://www.cmegroup.com/markets/equities/nasdaq/micro-e-mini-nasdaq-100.html#venue=globex')  
